...
**DeepSeek Harness（DSH）字体插件**：设置正文与代码的字体族、各自独立的字号与字重，并按「西文 / 中文」分栏挑字体；配置入口是 **设置 → 插件 → 插件配置** 里的原生卡片，改完即时生效。
...
- **正文字重** —— 300 ~ 600 任意整数，覆盖正文与界面文字的粗细（含标题）；未设置或 400 表示沿用 DSH 原本的字重
- **代码字重** —— 300 ~ 600 任意整数，只覆盖代码块、行内代码与终端输出；未设置或 400 表示代码保持 DSH 原样，也就是**不被正文字重带着走**
...
- **调整字号与字重** —— 滑块就在它作用的对象下面：正文字体 → 正文字号偏移 → 正文字重 → 代码字体 → 代码字号偏移 → 代码字重。拖动松手即生效；改过的字段标「已修改」并可单独重置，卡片底部有「全部重置」
