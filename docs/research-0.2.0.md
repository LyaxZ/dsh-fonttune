...
>
> **⚠️ 这是 0.2.0 开工前的调研快照**（当时插件还是 v0.1.6）。文中「现状」「竞品做法」「候选方向」都可能与最终实现不同；有一条实测结论后来被推翻并改写：文末提到的「界面字号百分比全局缩放」在 0.2.0 里**整条退役**——实测 DSH 的设置页 / 侧栏 / 工作区字号是上游 CSS 字面量，界面阶梯 token 只被会话区组件消费，唯一官方字号接口是「对话字号」。最终形态见 [README](../README.md) / [CHANGELOG](../CHANGELOG.md)。
