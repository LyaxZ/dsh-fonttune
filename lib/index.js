...
/**
 * The schema library, taken from the HOST whenever the host can be reached.
 *
 * `@deepseek-ai/schemastery` is an optional peer dependency — DSH supplies the
 * schema library, this package never ships its own copy into a profile. Node's
 * bare-specifier resolution does not know that: when this package is linked or
 * junctioned into a profile (the development setup) the specifier resolves
 * inside this package's own `node_modules`, i.e. to the devDependency. That is
 * a SECOND instance of the library, and — worse — a newer one than the running
 * host may carry: its `.volatile()` marking lands on a schema the rc line's
 * settings controller cannot digest, so the namespace is never served (no card
 * in the settings sheet, an empty first-frame stylesheet). Measured on
 * 0.1.5-rc.3: linked → nothing, installed → works.
 *
 * Resolving from the host first keeps one instance and one dialect, which is
 * exactly what an installed package gets. `process.argv[1]` is the host's own
 * entry (its `node_modules` holds the authoritative copy); anything resolving
 * back inside THIS package is skipped, so the offline suites — which have no
 * host — still fall through to the devDependency.
 * @returns {object} the schemastery export.
 */
function loadSchemastery() {
  const own = dirname(dirname(fileURLToPath(import.meta.url))) + sep;
  const bases = [process.argv[1], fileURLToPath(import.meta.url)];
  for (const base of bases) {
    if (typeof base !== "string" || base === "") continue;
    try {
      const from = createRequire(base);
      if (!from.resolve("@deepseek-ai/schemastery").startsWith(own)) {
        return from("@deepseek-ai/schemastery");
      }
    } catch {
      // Not resolvable from that anchor: try the next one.
    }
  }
